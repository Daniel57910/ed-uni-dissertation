...
def load_entry_df():
    ...
    df = dd.read_parquet(files[:1], usecols=INITIAL_LOAD_COLUMNS)
...
def test_within_session_stats(load_entry_df: Any):
    ...
    sample_session = df[(
        df['user_id'] == sample['user_id']) &
        (df['session_30'] == sample['session_30'])]

    sample_session = sample_session.sort_values(by='date_time')
   
    assert sample_session['cum_session_time_minutes'].max() == \
        (sample_session['date_time'].max() - sample_session['date_time'].min()).total_seconds() / 60
...
def test_running_user_stats(load_entry_df: Any):
    ...
    sample_user = df.iloc[random.randint(0, len(df))]['user_id']
    ...
    assert sample_df.iloc[-1]['cum_platform_time_minutes'] == (sample_df['delta_last_event'].sum() / 60)
    assert sample_df.iloc[-1]['cum_projects'] == sample_df['project_change'].sum() + 1
...
def test_session_stats(load_entry_df):
    ...
    sample_df = sample_df[['user_id', 'session_30', 'expanding_session_time_minutes', 'session_time_mins']].drop_duplicates()
    assert first_session['expanding_session_time_minutes'].max() == 0
    last_session = sample_df.iloc[-1]['expanding_session_time_minutes']
    assert last_session == sample_df['session_time_mins'].iloc[:-1].sum() / (sample_df.shape[0] - 1)
